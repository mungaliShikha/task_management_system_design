...
module.exports = {
  ...
  createManager: catchAsync(async (req, res) => {
    ...
    let { userId } = req.params;
    const adminAuthCheck = await User.findById(userId);
    if (adminAuthCheck && adminAuthCheck.role != "Admin") {
      throw new appError(ErrorMessage.INVALID_TOKEN, ErrorCode.NOT_ALLOWED);
    } else if (!adminAuthCheck) {
      throw new appError(ErrorMessage.USER_NOT_FOUND, ErrorCode.NOT_FOUND);
    }
    ...
    let subject = "Manager Invitation";
    let message = `Your account is successfully created as A Manager on Our plateform <br> Kindly Use this Credentials for Login <br> email:${req.body.email} <br> Password: ${passGen}`;
